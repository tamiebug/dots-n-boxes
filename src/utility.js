export class SquareGrid {
	...
	allPossibleMoves() {
		...
		for (let r = 0; r < this.nRows; r++) {
			for (let c = 0; c < this.nColumns; c++) {
				...
				if (this.isMovePossible(hMove)) moves.append(hMove);
				if (this.isMovePossible(vMove)) moves.append(vMove);
			}
		}
		return moves;
	}
	...
	moveIsWithinBounds(move) {
		if (this.isWithinBounds(move.r, move.c)) {
			if (move.r == this.nRows && move.isHorizontal()) return false;
			if (move.c == this.nColumns && move.isVertical()) return false;
			return true;
		}
		return false;
	}
	...
	isMovePossible(move) {
		return this.moveIsWithinBounds(move) && !this.hasMoveBeenDone(move);
	}
}
